...
# UCB hyperparameters
c = 2  # Exploration coefficient for UCB
...
# Initialize an empty Q-table and action count table
Q_table = {}
action_counts = {}
...
def choose_action_ucb(state_index):
    state_key = tuple(state_index)
    
    # Initialize Q-table and action counts if the state is unseen
    if state_key not in Q_table:
        Q_table[state_key] = np.zeros(np.prod(action_shape))
        action_counts[state_key] = np.ones(np.prod(action_shape))  # Start with all actions explored once
    
    # Calculate UCB values for each action
    total_count = sum(action_counts[state_key])
    ucb_values = Q_table[state_key] + c * np.sqrt(np.log(total_count) / action_counts[state_key])
    
    # Choose action with maximum UCB value
    return np.unravel_index(np.argmax(ucb_values), action_shape)
...
# Update action count for UCB calculation
def update_action_count(state_index, action_index):
    state_key = tuple(state_index)
    action_counts[state_key][np.ravel_multi_index(action_index, action_shape)] += 1
...
# Main SARSA loop with UCB action selection
last_reward = 0
...
for episode in range(num_episodes):
    ...
    actions = choose_action_ucb(state_index)
    ...
    # Choose the next action based on the new state index using UCB
    new_actions = choose_action_ucb(new_state_index)

    # Update Q-values using the SARSA update rule
    old_q_value = get_q_value(state_index, actions)
    new_q_value = old_q_value + alpha * (reward + gamma * get_q_value(new_state_index, new_actions) - old_q_value)
    update_q_value(state_index, actions, new_q_value)

    # Update action count for UCB
    update_action_count(state_index, actions)
    ...
    save_csv(dict_record, f'sarsa_logs.csv')

    # Track maximum reward and best config
    if reward > max_reward:
        max_reward = reward
        best_config = state_index
