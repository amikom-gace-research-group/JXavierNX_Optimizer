...
def save_csv(dict_list, filename):
    with open(filename, 'a', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['api_time','id', 'reward', 'phase', 'episode', 'xaviernx_time_elapsed', 'sarsa_time_elapsed', 'cpu_cores', 'cpu_freq', 'gpu_freq', 'memory_freq', 'cl', 'throughput', 'power_cons', 'cpu_percent', 'gpu_percent', 'mem_percent'])
...
for episode in range(num_episodes):
    if episode < 75:
        ...
        if actions:
            ...
            state_index = tuple(state_to_index(cpu_cores, cpu_freq, gpu_freq, memory_freq, cl))
            if state_index not in Q_table:
                Q_table[state_index] = np.zeros(np.prod(action_shape))
    else:
