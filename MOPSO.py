...
def save_csv(results, filename):
    ...
    with open(filename, 'a', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['api_time', 'episode', 'iteration', 'reward', 'xavier_time_elapsed', 'mopso_time_elapsed', 'cpu_cores', 'cpu_freq', 'gpu_freq', 'mem_freq', 'cl', 'throughput', 'power_cons', 'cpu_percent', 'gpu_percent', 'mem_percent'])
    ...
    def optimize(self):
        ...
        for iteration in range(self.max_iter):
            ...
            for particle in self.swarm:
                ...
                if episode == 1:
                    config = [
                        max(CPU_CORES_RANGE), max(CPU_FREQ_RANGE), max(GPU_FREQ_RANGE), max(MEMORY_FREQ_RANGE), max(CL_RANGE)
                    ]
                else:
                    config = [
